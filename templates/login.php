...
<?php if ($view->flashMessage->exists(FlashMessage::ERROR)) { ?>
    <div style="border: 1px solid #ccc; padding: 10px; margin:5px; border-radius: 5px;">
        <span>Error:</span>
        <code><?= $view->out($view->flashMessage->get(FlashMessage::ERROR)) ?></code>
    </div>
<?php } ?>
...
<div class="content_wrapper" style="padding: 0 20px">
    ...
    <form method=POST action="<?= $view->path($view->auth->config->loginRoute) ?>">
